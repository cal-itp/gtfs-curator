...
def get_vp_base64_url(rt_trips_file_path: str) -> pd.DataFrame:
    ...
    df = pd.read_parquet(
        rt_trips_file_path,
        columns=["schedule_base64_url", "vp_base64_url"],
        filesystem=gcsfs.GCSFileSystem(),
    ).drop_duplicates()
...
def download_schedule_table(
    table_name: str, date_list: list, operator_list: list, gcs_folder: str
):
    """
    ...
    """
    basic_query = bq_utils.basic_sql_query(
        "cal-itp-data-infra", "mart_gtfs", table_name
    )
    ...
    if table_name == "fct_scheduled_trips":
        ...
        feed_key_url_df = get_feed_keys_and_schedule_base64_url(
            f"{gcs_folder}fct_scheduled_trips.parquet"
        )
        gtfs_name_col = TABLE_KEYS_DICT[table_name]
        operator_filter_query = filter_by_operator_list(
            gtfs_name_col, feed_key_url_df[gtfs_name_col].unique()
        )

    # all these date filters are or statements, so wrap that up in parentheses
    df = download_table(
        f"{basic_query} WHERE ({date_filter_query}) AND {operator_filter_query}"
    )
    ...
    if table_name == "fct_daily_scheduled_stops":
        ...
        utils.geoparquet_gcs_export(
            df, gcs_folder, table_name, filesystem=gcsfs.GCSFileSystem()
        )
...
def download_vp_tables(
    ...
    gtfs_name_col = TABLE_KEYS_DICT[table_name]
    operator_filter_query = filter_by_operator_list(
        gtfs_name_col, subset_urls_df[gtfs_name_col].unique()
    )
    ...
    if table_name == "fct_vehicle_locations_path":
        ...
        df.to_parquet(
            f"{gcs_folder}{table_name}.parquet", filesystem=gcsfs.GCSFileSystem()
        )
...
if __name__ == "main":
    for one_table in hb.GTFS_MART_TABLES:
        download_schedule_table(
            one_table, hb.HERMOSA_BEACH_DATES, hb.SUBSET_OF_OPERATORS, HERMOSA_BEACH_GCS
        )
