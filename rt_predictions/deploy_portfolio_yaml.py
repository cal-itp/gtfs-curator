...
def check_stop_and_route_counts(one_month: str):
    ...
    stop_df = (
        report_utils.import_stop_df(
            filters=filtering, columns=["schedule_name", "route_name", "geometry"]
        )
...
def check_route_counts(one_month: str):
    ...
    operator_df = (
        report_utils.import_operator_df(
            filters=filtering, columns=["schedule_name", "tu_name"]
        )
        .drop_duplicates()
        .reset_index(drop=True)
    )
    ...
    # Use inner merge - charts and maps need both dfs to work
    count_df = pd.merge(
        operator_df, route_df, on=["schedule_name", "tu_name"], how="inner"
    )
...
def overwrite_yaml(name: str = typer.Argument(default="rt_msa"), month: str = ""):
    ...
    if name == "rt_msa_stops":
        ...
        portfolio_utils.create_portfolio_yaml_chapters_no_sections(
            ...
            chapter_name="name",
            chapter_values=sorted(list(df.tu_name)),
        )
