...
WITH daily_schedule AS (
    ...
    gtfs_dataset_key IN ('70278036918ea1640d84a4f7c1d6af66', '0089bd1b0a2b78a8590d8749737d7146', '9b4a37e507b9d86a115dbcc6bb764a65')
),

daily_rt AS (
    SELECT *
    FROM `cal-itp-data-infra.mart_gtfs.fct_daily_rt_service_summary`
    WHERE service_date >= "2026-02-01" AND service_date < "2026-03-01" AND schedule_base64_url IN (
        'aHR0cHM6Ly9tYXJpbnRyYW5zaXQubmV0L2d0ZnM=',
        'aHR0cHM6Ly9hcGkuNTExLm9yZy90cmFuc2l0L2RhdGFmZWVkcz9vcGVyYXRvcl9pZD1NQQ==',
        'aHR0cHM6Ly9tYXJpbnRyYW5zaXQub3JnL2RhdGEvZ29vZ2xlX3RyYW5zaXQuemlw')
    ),
),
...
daily_tu_stop_metrics AS (
    SELECT *
    FROM `cal-itp-data-infra-staging.tiffany_mart_gtfs.fct_trip_updates_stop_metrics_marin`
),
