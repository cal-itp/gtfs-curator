...
def prep_fct_vp_stop_metrics(
    ...
    stop_gdf = stop_gdf.assign(
        pct_vp_trips=stop_gdf.n_vp_trips.divide(stop_gdf.n_trips).round(3)
    )
...
def prep_vp_path(
    file_name: str = GTFS_DATA_DICT.speeds_tables.vp_path, **kwargs
) -> pd.DataFrame:
    """
...
def prep_intermediate_vp_stops_trip_crosswalk(
    file_name: str = "int_gtfs_rt__vehicle_positions_trip_stop_day_map_grouping",
    **kwargs,
) -> pd.DataFrame:
    """
...
def filter_to_potential_detour_stops(
    ...
    intermediate_crosswalk: pd.DataFrame,
    vp_path: gpd.GeoDataFrame,
    cutoffs: list,
) -> gpd.GeoDataFrame:
    """
    ...
    gdf = (
        ...
        .merge(
            intermediate_crosswalk,
            on=["service_date", "feed_key", "vp_base64_url", "stop_id"],
            how="inner",
        )
        .merge(
            vp_path.rename(
