...
def find_percentiles(df: pd.DataFrame, col: str = "Route Length Miles") -> pd.DataFrame:
    """
...
def reshape_percentile_groups(df: pd.DataFrame) -> pd.DataFrame:
    ...
    agg1 = (
        ...
        .reset_index()
    ).rename(columns={"Route Name": "Total Routes"})
    return agg1
...
def create_route_lengths(df: pd.DataFrame):
    df2 = find_percentiles(df, col="Route Length Miles")
